...
import BackgroundSpace from "./backgroundLogin";
import ipad from "../../../assets/img/svg/Ipad.svg";
...
export default function Login(props) {
  ...
  const { loading, error, data, executeService } = useFreeApi(
    "/user/login",
    "post"
  );
  ...
  const {
    ...
    handleBlur,
    classNames,
  } = useForm(submitForm, getDefaultValues(), loginSchema);
  useEffect(() => {
              ...
              <form onSubmit={handleSubmit} noValidate>
                ...
                <div style={{ textAlign: "center", marginTop: 10 }}>
                  <SubmitLoginButton type="submit">
                    {loading ? (
                      <BeatLoader size={10} color="#F1EAE0" loading />
                    ) : (
                      "Login"
                    )}
                  </SubmitLoginButton>
